...
static Arduino_Canvas*   _tgt = nullptr;
...
void characterRenderTo(Arduino_Canvas* tgt, int cx, int cy) {
  if (!gifOpen) return;   // caller opens via characterSetState(activeState)
  Arduino_Canvas* prevT = _tgt; bool prevP = peekMode; int px = gifX, py = gifY;
...
void characterTick() {
  ...
  if (textMode) {
    ...
    _tgt->fillRect(0, cy - 14, _tgt->width(), 28, pal.bg);

    const char* line = ts.frames[textFrame];
    int len = strlen(line);
    int tw = len * 12;                                    // size-2 glyph width
    _tgt->setTextColor(pal.body, pal.bg);
    _tgt->setTextSize(2);
    _tgt->setCursor((_tgt->width() - tw) / 2, cy - 8);
    _tgt->print(line);
